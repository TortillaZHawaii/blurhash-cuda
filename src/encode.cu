...
static int encodeAC(float r, float g, float b, float maximumValue)
{
	int quantR = fmaxf(0, fminf(18, floorf(signedPow(r / maximumValue, 0.5) * 9 + 9.5)));
	int quantG = fmaxf(0, fminf(18, floorf(signedPow(g / maximumValue, 0.5) * 9 + 9.5)));
	int quantB = fmaxf(0, fminf(18, floorf(signedPow(b / maximumValue, 0.5) * 9 + 9.5)));
